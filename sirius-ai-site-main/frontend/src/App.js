...
function App() {
  ...
  const handleSendMessage = async () => {
    ...
    // Try backend AI first, fall back to local keyword matching
    let aiResult = null;
    try {
      ...
      // Backend not available - use local AI
      console.log("Using local AI response");
      aiResult = generateLocalAIResponse(userMessage, sightings);
    }
    ...
    if (aiResult.location) {
      setHighlightedLocation({
        latitude: aiResult.location.latitude,
        longitude: aiResult.location.longitude,
        title: aiResult.location.event_title || aiResult.location.location
      });
      toast.success(`Location found: ${aiResult.location.location}`);
    }
