 ...
 * - Caches result in localStorage so reloads don't blow YouTube quota
...
function LiveMediaFooter() {
  ...
  const [activeVideo, setActiveVideo] = useState(null); // inline player
  const [lastUpdated, setLastUpdated] = useState(null);

  // --- Fetch live feed from /api/youtube-feed (with 12h cache) ---
  const loadFeed = useCallback(async (forceRefresh = false) => {
    if (!forceRefresh) {
      try {
        const cached = JSON.parse(localStorage.getItem(LIVE_FEED_CACHE_KEY) || 'null');
        if (cached && cached.timestamp && (Date.now() - cached.timestamp) < LIVE_FEED_TTL_MS && Array.isArray(cached.videos) && cached.videos.length > 0) {
    ...
    setLoading(true);
    try {
      const res = await axios.get('/api/youtube-feed', { timeout: 20000 });
      ...
      if (items.length > 0) {
        ...
        setLastUpdated(ts);
        try {
          localStorage.setItem(LIVE_FEED_CACHE_KEY, JSON.stringify({ timestamp: ts, videos: normalized }));
        } catch (e) { /* quota */ }
      } else {
        setVideos(VIDEO_DATA);
      }
    } catch (e) {
      console.warn('[LiveMediaFooter] YouTube feed fetch failed, using fallback:', e?.message);
      setVideos(VIDEO_DATA);
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    loadFeed(false);
    const interval = setInterval(() => loadFeed(true), LIVE_FEED_TTL_MS);
  ...
  const handleMouseMove = (e) => {
    ...
    if (Math.abs(walk) > 5) setMovedDuringDrag(true);
  ...
  return (
    <div
      className="fixed bottom-0 left-0 w-full bg-slate-950/95 backdrop-blur-md border-t border-cyan-500/30 px-4 pt-3 pb-3 z-50 select-none"
      data-testid="live-media-footer"
    >
      {/* Header */}
      <div className="flex items-center justify-between mb-2 px-1">
        <div className="text-cyan-400 font-bold flex items-center gap-2 text-xs uppercase tracking-wider">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            disabled={loading}
            className="text-cyan-400/70 hover:text-cyan-300 transition-colors disabled:opacity-40"
      ...
      {/* Inline Player (in place, above thumbnails) */}
      {activeVideo && (
        <div
          className="mb-3 relative w-full max-w-3xl mx-auto rounded-lg overflow-hidden border border-cyan-500/40 bg-black shadow-lg shadow-cyan-500/10"
          data-testid="live-feed-inline-player"
        >
          <button
            onClick={closeInlinePlayer}
            className="absolute top-2 right-2 z-10 bg-slate-900/90 hover:bg-red-600 text-white rounded-full w-7 h-7 flex items-center justify-center text-xs transition-colors"
            data-testid="live-feed-close-player-btn"
            title="Close player"
          >
            <X size={14} />
          </button>
          ...
          <div className="px-3 py-2 bg-slate-900/95 border-t border-cyan-500/20">
            <p className="text-white text-xs font-medium truncate">{activeVideo.title}</p>
            {activeVideo.channel && (
              <p className="text-gray-500 text-[10px] truncate">{activeVideo.channel}</p>
            )}
          </div>
        </div>
      )}
      ...
      <div
        ...
        className={`flex gap-3 overflow-x-auto scrollbar-none pb-1 ${isDragging ? 'cursor-grabbing' : 'cursor-grab'}`}
        style={{ scrollBehavior: isDragging ? 'auto' : 'smooth' }}
      >
        {videos.map((video, idx) => {
          const isActive = activeVideo && activeVideo.videoId === video.videoId;
          return (
            <button
              type="button"
              ...
              className={`flex-shrink-0 w-56 text-left bg-slate-900/80 border rounded-lg p-2 transition-all group ${
                isActive
                  ? 'border-cyan-400 ring-2 ring-cyan-500/40'
                  : 'border-slate-800 hover:border-cyan-500/50 hover:bg-slate-800/80'
              }`}
            >
              <div className="relative w-full h-28 rounded-md overflow-hidden bg-slate-800 mb-2">
                ...
                {isActive && (
                  <div className="absolute top-1 left-1 bg-cyan-500 text-black text-[9px] font-bold px-1.5 py-0.5 rounded uppercase tracking-wide">
                    Now Playing
                  </div>
                )}
              </div>
              ...
              {video.channel && (
                <p className="text-gray-500 text-[10px] truncate mt-0.5">{video.channel}</p>
              )}
            </button>
          );
        })}
